...
fun JetProperty.names(writeable: Boolean = true): List<String> {
    val name = getName()!!
    ...
    if (writeable) return lightMethods.mapTo(arrayListOf(name)) { it.getName() }

    val getter = lightMethods.getGetter()
    return if (getter != null) arrayListOf(name, getter.getName()) else Collections.singletonList(getName())
}
...
public abstract class UsagesSearchHelper<T : PsiNamedElement>() {
    ...
    protected open fun makeWordList(target: UsagesSearchTarget<T>): List<String> = with(target) {
        val name = element.getName()!!

        fun withPropertyAccessorRequests(property: JetProperty): List<String> =
                LightClassUtil.getLightClassPropertyMethods(property).mapTo(ArrayList<String>()) { it.getName() }
...
class ClassUsagesSearchHelper() : DeclarationUsagesSearchHelper<JetClassOrObject>() {
    ...
    override fun makeFilter(target: UsagesSearchTarget<JetClassOrObject>): UsagesSearchFilter =
            isTargetUsage and isFilteredImport and when {
    ...
    override fun makeItemList(target: UsagesSearchTarget<JetClassOrObject>): List<UsagesSearchRequestItem> {
        ...
        for (decl in target.element.getDeclarations()) {
            if ((decl is JetNamedFunction && functionUsages) || (decl is JetProperty && propertyUsages)) {
                items.add(DefaultUsagesSearchHelper.newItem(target.retarget(decl as JetNamedDeclaration)))
            }
        }
...
class PropertyUsagesSearchHelper() : CallableUsagesSearchHelper<JetProperty>() {
    ...
    override fun makeWordList(target: UsagesSearchTarget<JetProperty>): List<String> {
        return target.element.names(writeUsages)
    }
